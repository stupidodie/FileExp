...
void CreateTree(HufNode *huf_tree, unsigned int char_kinds, unsigned int node_num)
{
    ...
    for (i = char_kinds; i < node_num; ++i)
    {
        ...
        huf_tree[s1].parent = huf_tree[s2].parent = (int)i;
...
void HufCode(HufNode *huf_tree, unsigned char_kinds)
{
    ...
    for (i = 0; i < char_kinds; ++i)
    {
        index = 256 - 1; // 编码临时空间索引初始化
                         // 从叶子向根反向遍历求编码
        for (cur = (int)i, next = huf_tree[i].parent; next != 0;
...
** 统计并拷贝到树结点后立即释放
*/
    auto *tmp_nodes = (TmpNode *)malloc(256 * sizeof(TmpNode));
    // 初始化暂存结点
    for (i = 0; i < 256; ++i)
    {
    ...
    // 判断输入文件是否存在
    assert(infile != nullptr); //infile not exist!
    ...
    if ((number != 0) && (static_cast<size_t>(number) == password.length()))
    {
        //如果密码位数不是0位
        //将密码添加进去
        char pwd[number];
        strcpy(pwd, password.c_str());
        fwrite(pwd, number, 1, outfile);
    }
    ...
    else
    {
        ...
        while (!feof(infile))
        {
            ...
            while (strlen(code_buf) >= 8)
            {
                ...
                for (i = 0; i < 8; ++i)
                {
                    char_temp <<= 1u; // 左移一位，为下一个bit腾出位置
                    if (code_buf[i] == '1')
                        char_temp |= 1u; // 当编码为"1"，通过或操作符将其添加到字节的最低位
                }
...
bool extract(const char *ifname, const char *ofname, const char *password)
{
    ...
    fread(&number, sizeof(number), 1, infile);
    if (number != 0)
    {
        char buf[number];
        fread(buf, number, 1, infile);
    ...
    else
    {
        ...
        while (true)
        {
            ...
            for (i = 0; i < 8; ++i)
            {
                ...
                code_temp <<= 1u; // 将编码缓存的下一位移到最高位，供匹配
            }
            if (writen_len == file_len)
                break; // 控制文件长度，跳出外层循环
        }
